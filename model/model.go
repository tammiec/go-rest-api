...
// 	users := make([]*User, 0)
// 	rows, err := db.Query(context.Background(), "SELECT id, name, email, password FROM users")
// 	if err != nil {
...
// 	user := &User{}
// 	err := db.QueryRow(context.Background(), fmt.Sprintf("DELETE FROM users WHERE id=%v RETURNING id, name, email, password", id)).Scan(&user.Id, &user.Name, &user.Email, &user.Password)
// 	if err != nil {
// 		fmt.Fprintf(os.Stderr, "Row error: %v\n", err)
// 		os.Exit(1)
// 	}
